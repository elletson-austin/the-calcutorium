...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        left_panel, self.function_editors_layout = make_left_panel(
            self.input_win, self.output_widget
        )
        main_layout.addWidget(left_panel)
        main_layout.addWidget(self.render_widget, 1)

        # Connect render_widget signal
        self.render_widget.manual_range_cleared.connect(
            self._handle_manual_range_cleared
        )
    ...
    def _handle_manual_range_cleared(self):
        self.output_widget.write(
            "Manual range cleared due to user interaction. Returning to automatic ranging."
        )
    ...
    def update_function_editors(self):
        ...
        sync_function_editors(
            self.scene, self.function_editors, self.function_editors_layout, factory
        )

    def on_equation_changed(self, math_function: MathFunction, new_equation: str):
        self.output_widget.write(
            f"Equation changed for '{math_function.name}': '{new_equation}'"
        )
        ...
        if not success:
            ...
            if editor:
                editor.equation_input.setText(
                    getattr(math_function, "equation_str", "")
                )
